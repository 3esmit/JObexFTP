        ...
        for (int i = 0; i < args.length; i++) {
            if ("-p".equals(args[i]) || "--portname".equals(args[i])) {
                ...
                try {
                    ...
                    Logger.getLogger("com.lhf.jobexftp").severe("Could not convert baudrate value. Please insert only numbers");
                }
            } else if ("-w".equals(args[i]) || "--wait".equals(args[i])) {
                try {
                    timeout = Integer.parseInt(args[i + 1]);
                } catch (NumberFormatException ex) {
                    Logger.getLogger("com.lhf.jobexftp").severe("Could not convert timeout value. Please insert only numbers");
                }
                ...
                if (w.startsWith("none")) {
                    ...
                    Logger.getLogger("com.lhf.jobexftp").severe("Invalid flow control.");
                }
            } else if ("-d".equals(args[i]) || "--debug".equals(args[i])) {
                Log.logLevel = Log.LOG_DEBUG;
                Utility.configLogger(Level.FINEST);
                //Logger.getLogger("com.lhf.jobexftp").setLevel(Level.FINEST);
            } else if ("-q".equals(args[i]) || "--quiet".equals(args[i])) {
                Log.logLevel = Log.LOG_NONE;
                Logger.getLogger("com.lhf.jobexftp").setLevel(Level.SEVERE);
